...
기능: 관리자 사용자 역할 변경 API
  ...
  시나리오: 마스터가 사용자를 담당자로 변경한다
    먼저 마스터가 로그인되어 있다
    만일 마스터가 사용자 5의 역할을 "MANAGER"로 변경한다
    그러면 역할 변경이 성공한다
    그리고 변경된 역할이 "MANAGER"이다

  시나리오: 마스터가 담당자를 일반으로 변경한다
    먼저 마스터가 로그인되어 있다
    만일 마스터가 사용자 5의 역할을 "USER"로 변경한다
    그러면 역할 변경이 성공한다
    그리고 변경된 역할이 "USER"이다

  시나리오: 성전이 없는 사용자를 담당자로 변경하면 실패한다
    먼저 마스터가 로그인되어 있다
    만일 마스터가 사용자 1의 역할을 "MANAGER"로 변경한다
    그러면 Bad Request 에러가 반환된다
